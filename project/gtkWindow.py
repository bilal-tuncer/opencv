...
class Scale(Gtk.Window):
    ...
    def __init__(self,in_fp,video_time,org_fps):
        ...
        out_grid.attach(grid,0,0,1,1)

        scrolledwindow.add(out_grid)
        self.org_fps = org_fps
        label1 = Gtk.Label(label="Chose an interval to cut the video")
        grid.attach(label1,0,1,2,1)

        self.scale = Gtk.Scale().new_with_range(orientation=Gtk.Orientation.HORIZONTAL,min =0.0,max = video_time, step=0.001)
        ...
        grid.attach(self.fps_entry,1,10,1,1)
        ...
        #vadjustment = grid.get_vadjustment()
        vscrollbar = Gtk.Scrollbar(orientation=Gtk.Orientation.VERTICAL,)
        out_grid.attach(vscrollbar, 1, 0, 1, 1)
    ...
    def reframe(self,scale):
        pic = self.frames[minutes_to_frame(self.scale.get_value(),self.org_fps)]
        pic_xsize = 335
        pic_ysize = 270
        pic = cv.resize(pic, (pic_xsize,pic_ysize))
        pic = cv.cvtColor(pic,cv.COLOR_BGR2RGB)
